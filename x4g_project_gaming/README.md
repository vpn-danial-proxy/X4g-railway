...
## 🎮 پریست گیمینگ (کمترین لتنسی ممکن روی همین زیرساخت)

علاوه بر ساخت کانفیگ معمولی، یک endpoint اختصاصی اضافه شده که یک کانفیگ با تنظیمات بهینه برای گیمینگ می‌سازه:

```
POST /api/links/gaming
Body (JSON): { "label": "PS5", "limit_value": 0, "expires_days": 0, "ip_limit": 0, "port": 443 }
```

این endpoint همیشه:
* پروتکل رو روی `vless-ws` قفل می‌کنه (کمترین سربار در مقایسه با هر دو مد XHTTP)
* `speed_limit_bytes` رو صفر نگه می‌داره (بدون Token Bucket / تأخیر مصنوعی)

⚠️ **این پریست فقط سربار قابل‌کنترل روی همین کد رو کمینه می‌کنه، نه یک عدد پینگ مشخص.** پینگ نهایی به مسیر فیزیکی اینترنت شما تا ریجن Railway بستگی داره؛ برای بازی‌هایی که ترافیک اصلی‌شون UDP هست، تونل کردن روی TCP همچنان یعنی زیر پکت‌لاس بالا ممکنه جهش لتنسی داشته باشید — این محدودیت پروتکلیه و با هیچ تنظیمی از بین نمی‌ره. برای کمترین پینگ، ریجن Railway را روی **Europe West** بگذارید و در صورت نیاز با Asia Southeast مقایسه کنید.
